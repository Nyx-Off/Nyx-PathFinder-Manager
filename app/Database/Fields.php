...
final class Fields
{
    ...
    public static function split(string $table, array $data): array
    {
        $columns = [];
        foreach (self::MAP[$table] ?? [] as $key => $type) {
            $columns[$key] = $data[$key] ?? null;
            unset($data[$key]);
            if (is_bool($columns[$key])) {
                $columns[$key] = (int)$columns[$key];
            }
        }
        $columns['data'] = json_encode($data, JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR);
        return $columns;
    }
    ...
    public static function hydrate(string $table, array $row): array
    {
        ...
        foreach (self::MAP[$table] ?? [] as $key => $type) {
            if (isset($row[$key])) {
                $d[$key] = $type === 'INTEGER' ? (int)$row[$key] : ($type === 'DECIMAL(10,2)' ? (float)$row[$key] : $row[$key]);
            }
        }
        return $d;
    }
