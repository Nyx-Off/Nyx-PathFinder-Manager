...
spl_autoload_register(function ($class) { $path = ROOT . '/' . str_replace('\\', '/', $class) . '.php'; if (is_file($path)) require $path; });
...
if (PHP_SAPI !== 'cli') {
 ...
 ini_set('session.use_strict_mode','1'); ini_set('session.use_only_cookies','1'); session_save_path(ROOT.'/storage/sessions'); session_name('nyx_pathfinder'); session_set_cookie_params(['httponly'=>true,'secure'=>!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off','samesite'=>'Strict','path'=>rtrim(dirname($_SERVER['SCRIPT_NAME']),'/').'/']); session_start();
