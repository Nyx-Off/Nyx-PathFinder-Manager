...
$lock=fopen(ROOT.'/storage/operations.lock','c');if(!$lock||!flock($lock,LOCK_EX))exit(1);
...
try {
 ...
 $zip=new ZipArchive();$zip->open($base.'/uploads.zip',ZipArchive::CREATE);$zip->addEmptyDir('portraits');foreach(glob(ROOT.'/storage/uploads/*') as $file)if(is_file($file))$zip->addFile($file,basename($file));$zip->close();echo "Sauvegarde créée : storage/backups/$stamp\n";
