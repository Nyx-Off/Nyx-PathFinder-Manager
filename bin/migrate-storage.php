...
try {
    ...
    if ($mode === '--check') {
        echo "Connexion MySQL validée. Aucun changement de base active.\n";
        exit;
    }
    ...
    foreach ($source->query("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%' ORDER BY rowid")->fetchAll(PDO::FETCH_COLUMN) as $table) {
        ...
        if (in_array($table, ['app_settings','schema_migrations'])) {
            $mysql->exec("DELETE FROM `$table`");
        }
        $query = $source->query("SELECT * FROM `$table`");
        $statement = null;
        while ($row = $query->fetch()) {
            $statement ??= $mysql->prepare("INSERT INTO `$table` (`" . implode('`,`', array_keys($row)) . "`) VALUES (" . implode(',', array_fill(0, count($row), '?')) . ")");
            $statement->execute(array_values($row));
        }
        $src = (int)$source->query("SELECT COUNT(*) FROM `$table`")->fetchColumn();
        $dst = (int)$mysql->query("SELECT COUNT(*) FROM `$table`")->fetchColumn();
        if ($src !== $dst) {
    ...
    $env = preg_replace('/^DB_DRIVER=.*$/m', 'DB_DRIVER=mysql', $env);
    file_put_contents($path . '.next', $env, LOCK_EX);
    chmod($path . '.next', 0600);
    rename($path . '.next', $path);
    echo "Copie vérifiée et MySQL activé. La base SQLite d'origine est conservée.\n";
} catch (Throwable $e) {
    if (isset($mysql) && $mysql->inTransaction()) {
        $mysql->rollBack();
    }
    if (isset($source) && $source->inTransaction()) {
        $source->rollBack();
    }fwrite(STDERR, "Opération interrompue ; aucune activation MySQL. Vérifiez la connexion et que la destination est vide. Un échec après création du schéma peut laisser une destination initialisée : ne la supprimez pas sans contrôle.\n");
    exit(1);
}
