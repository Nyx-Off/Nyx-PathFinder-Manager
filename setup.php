...
try {
    ...
    $allowed = (int)$db->query('SELECT COUNT(*) FROM users')->fetchColumn() === 0 && env('SETUP_TOKEN_HASH') !== '';
    if (!$allowed) {
        http_response_code(403);
        throw new DomainException('L’initialisation est fermée. Connectez-vous avec votre compte.');
    }
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        if (!hash_equals($_SESSION['csrf'], $_POST['csrf'] ?? '') || !hash_equals(env('SETUP_TOKEN_HASH'), hash('sha256', $_POST['token'] ?? ''))) {
            throw new DomainException('Clé d’initialisation invalide. Utilisez le lien privé fourni.');
        }
        $email = trim($_POST['email'] ?? '');
        $password = $_POST['password'] ?? '';
        if (!filter_var($email, FILTER_VALIDATE_EMAIL) || strlen($email) > 190 || strlen($password) < 12 || strlen($password) > 200) {
            throw new DomainException('Adresse valide et mot de passe de 12 à 200 caractères requis.');
        }
        $db->beginTransaction();
        $q = $db->prepare("UPDATE app_settings SET setting_value='1' WHERE setting_key='setup_complete' AND setting_value='0'");
        $q->execute();
        if ($q->rowCount() !== 1) {
            throw new DomainException('Initialisation déjà effectuée.');
        }
        if ((int)$db->query('SELECT COUNT(*) FROM users')->fetchColumn() !== 0) {
            throw new DomainException('Un compte existe déjà.');
        }
    ...
    $error = 'Initialisation momentanément indisponible.';
}
